...
(function () {
  ...
  add('ja', {
    ...
    'rua.strategy.body': 'RUA サービスの意味を明確にし、「受信 -> 解析 -> 集計/可視化」の安全なパイプラインを最優先に設計する.特にプライバシーと悪用耐性を先に固める.',
    'rua.keypoints.title': 'Key points',
    'rua.keypoints.li1': 'RUA は DMARC の集計レポート送信先を示すタグ（mailto URI）.',
    'rua.keypoints.li2': 'RUA サービスはレポートメールの受信・XML 解析・集計・可視化を担う（自社運用 or 外部委託）.',
    'rua.keypoints.li3': '主要リスクは DNS ではなく、データ取り扱い・マルチテナント隔離・悪用（過大/圧縮爆弾/洪水）.',

    'rua.definition.title': 'Definition-level clarity',
    'rua.definition.li1.html': 'DMARC の <span class="code">rua=</span> は aggregate data の送信先（URI のリスト）.未指定なら集計レポートは送られない.',
    'rua.definition.li2': '集計レポートは XML（RFC 7489 Appendix C）で、送信元IP・認証結果・alignment などを要約.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> は失敗/フォレンジックで、個別メッセージ情報を含み得るためプライバシー面で慎重対応が必要.',

    'rua.priorities.title': 'Prioritized checklist',
    'rua.priorities.note': 'P0 = 信頼性の最低ライン / P1 = 実運用の防衛力 / P2 = 仕上げ.',
    ...
    'rua.p0.li2': '受信の堅牢化: メール/添付サイズ上限、解凍サイズ上限、ネスト深度制限、タイムアウトで zip 爆弾を遮断.',
    'rua.p0.li3': 'XML 解析は XXE 無効化・DTD拒否、スキーマ/整合性検証で異常を早期破棄.',
    'rua.p0.li4': '保管と保持を明文化（生 XML は短期 or 非保存、IP は要配慮情報として扱う）.',
    'rua.p1.title.html': '<strong>P1 - 実運用で耐える</strong>',
    'rua.p1.li1': 'データ最小化モード: 標準/匿名化（IPv4 /24, IPv6 /48）/集計のみを選択可能にする.',
    'rua.p1.li2': 'マルチテナント隔離とアクセス制御（行レベル分離、暗号化、監査ログ）.',
  ...
  const ruaEn = {
    ...
    'rua.strategy.title': 'Strategy',
    'rua.strategy.body': 'Clarify what a RUA service means in DMARC, then prioritize a safe receive -> parse -> analyze pipeline with privacy and abuse resistance first.',
    'rua.keypoints.title': 'Key points',
    'rua.keypoints.li1': 'RUA is the DMARC tag that specifies aggregate report destinations (mailto URIs).',
    'rua.keypoints.li2': 'A RUA service receives report emails, parses XML, aggregates metrics, and presents dashboards (self-hosted or third party).',
    ...
    'rua.definition.li2': 'Aggregate reports are XML (RFC 7489 Appendix C) summarizing auth/alignment results per source IP and counts.',
    ...
    'rua.priorities.title': 'Prioritized checklist',
    'rua.priorities.note': 'P0 = baseline credibility / P1 = operational defense / P2 = polish.',
    'rua.p0.title.html': '<strong>P0 - Don\'t get people in trouble</strong>',
    ...
    'rua.p0.li2': 'Harden intake: size limits, decompressed size limits, nested archive depth limits, timeouts to block zip bombs.',
    'rua.p0.li3': 'Parse XML safely (XXE disabled, DTD rejected) and validate schema/semantics; drop malformed reports.',
    'rua.p0.li4': 'Publish explicit privacy + retention rules (raw XML short-term or none; treat IPs as sensitive).',
    'rua.p1.title.html': '<strong>P1 - Make it operationally defensible</strong>',
    'rua.p1.li1': 'Data minimization modes: standard / anonymized (IPv4 /24, IPv6 /48) / aggregates-only.',
    'rua.p1.li2': 'Multi-tenant isolation + access control (row-level separation, encryption, audit logs).',
  ...
  add('vi', {
    ...
    'rua.strategy.title': 'Chien luoc',
    'rua.strategy.body': 'Lam ro y nghia dich vu RUA trong DMARC, sau do uu tien pipeline an toan receive -> parse -> analyze, dat quyen rieng tu va chong lam dung len truoc.',
    'rua.keypoints.title': 'Cac diem chinh',
    'rua.keypoints.li1': 'RUA la the DMARC chi dinh dich den bao cao tong hop (URI mailto).',
    'rua.keypoints.li2': 'Dich vu RUA nhan email bao cao, parse XML, tong hop chi so va hien thi dashboard (tu van hanh hoac ben thu ba).',
    'rua.keypoints.li3': 'Rui ro chinh khong phai DNS ma la xu ly du lieu, cach ly tenant va lam dung (payload qua lon, zip bomb, report flood).',
    'rua.definition.title': 'Lam ro o muc dinh nghia',
    'rua.definition.li1.html': 'Trong DMARC, <span class="code">rua=</span> la "Reporting URI(s) for aggregate data" (danh sach URI). Neu thieu, bao cao tong hop se khong duoc tao.',
    'rua.definition.li2': 'Bao cao tong hop la XML (RFC 7489 Appendix C) tom tat ket qua auth/alignment theo IP nguon va so luong.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> khac: bao cao forensic/that bai co the chua du lieu theo tung thu va nhay cam hon ve rieng tu.',
    'rua.priorities.title': 'Danh sach uu tien',
    'rua.priorities.note': 'P0 = tin cay toi thieu / P1 = phong thu van hanh / P2 = hoan thien.',
    'rua.p0.title.html': '<strong>P0 - Tranh gay rac roi</strong>',
    'rua.p0.li1.html': 'Xac minh so huu domain (vi du TXT tai <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) truoc khi hien thi du lieu bao cao.',
    'rua.p0.li2': 'Gia co dau vao: gioi han kich thuoc, gioi han giai nen, gioi han do sau archive long nhau, timeout chan zip bomb.',
    'rua.p0.li3': 'Parse XML an toan (tat XXE, tu choi DTD) va kiem tra schema/nghia; loai bo bao cao loi.',
    'rua.p0.li4': 'Cong bo quy tac rieng tu va luu giu ro rang (XML tho ngan han hoac khong luu; coi IP la nhay cam).',
    'rua.p1.title.html': '<strong>P1 - Dam bao kha nang phong thu van hanh</strong>',
    'rua.p1.li1': 'Che do toi thieu hoa du lieu: tieu chuan / an danh (IPv4 /24, IPv6 /48) / chi tong hop.',
    'rua.p1.li2': 'Cach ly multi-tenant + kiem soat truy cap (tach theo hang, ma hoa, audit log).',
  ...
  add('th', {
    ...
    'rua.strategy.title': 'กลยุทธ์',
    'rua.strategy.body': 'ทำความชัดเจนว่า RUA service ใน DMARC หมายถึงอะไร แล้วจัดลำดับความสำคัญของ pipeline รับ -> parse -> วิเคราะห์ที่ปลอดภัย โดยให้ความเป็นส่วนตัวและการต้านการใช้งานผิดก่อน.',
    'rua.keypoints.title': 'ประเด็นสำคัญ',
    'rua.keypoints.li1': 'RUA คือแท็ก DMARC ที่ระบุปลายทางรายงานแบบ aggregate (mailto URI).',
    'rua.keypoints.li2': 'บริการ RUA รับอีเมลรายงาน, parse XML, รวมสถิติ และแสดงแดชบอร์ด (โฮสต์เองหรือบุคคลที่สาม).',
    'rua.keypoints.li3': 'ความเสี่ยงหลักไม่ใช่ DNS แต่คือการจัดการข้อมูล การแยกผู้เช่า และการถูกโจมตี (payload ใหญ่, zip bomb, flood ของรายงาน).',
    'rua.definition.title': 'ความชัดเจนระดับนิยาม',
    'rua.definition.li1.html': 'ใน DMARC, <span class="code">rua=</span> คือ "Reporting URI(s) for aggregate data" (รายการ URI). หากไม่มี จะไม่สร้างรายงาน aggregate.',
    'rua.definition.li2': 'รายงาน aggregate เป็น XML (RFC 7489 Appendix C) สรุปผล auth/alignment ต่อ IP ต้นทางและจำนวน.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> ต่างออกไป: รายงาน forensic/ล้มเหลวที่อาจมีข้อมูลต่อข้อความและไวต่อความเป็นส่วนตัวมากกว่า.',
    'rua.priorities.title': 'เช็กลิสต์ตามลำดับความสำคัญ',
    'rua.priorities.note': 'P0 = ความน่าเชื่อถือขั้นต่ำ / P1 = การป้องกันเชิงปฏิบัติการ / P2 = เก็บรายละเอียด.',
    'rua.p0.title.html': '<strong>P0 - อย่าให้เกิดปัญหา</strong>',
    'rua.p0.li1.html': 'ยืนยันความเป็นเจ้าของโดเมน (เช่น TXT ที่ <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) ก่อนแสดงข้อมูลรายงาน.',
    'rua.p0.li2': 'ทำให้การรับแข็งแรง: จำกัดขนาด, จำกัดขนาดหลังแตกไฟล์, จำกัดความลึกของไฟล์ซ้อน, ตั้ง timeout กัน zip bomb.',
    'rua.p0.li3': 'Parse XML อย่างปลอดภัย (ปิด XXE, ปฏิเสธ DTD) และตรวจ schema/ความหมาย; ทิ้งรายงานผิดรูป.',
    'rua.p0.li4': 'ประกาศกฎความเป็นส่วนตัวและการเก็บรักษาชัดเจน (XML ดิบระยะสั้นหรือไม่เก็บ; ถือว่า IP เป็นข้อมูลอ่อนไหว).',
    'rua.p1.title.html': '<strong>P1 - ป้องกันเชิงปฏิบัติการได้</strong>',
    'rua.p1.li1': 'โหมดลดข้อมูล: มาตรฐาน / ทำให้ไม่ระบุตัวตน (IPv4 /24, IPv6 /48) / เฉพาะรวมยอด.',
    'rua.p1.li2': 'แยกผู้เช่า + ควบคุมการเข้าถึง (แยกระดับแถว, เข้ารหัส, audit log).',
  ...
  add('id', {
    ...
    'rua.strategy.title': 'Strategi',
    'rua.strategy.body': 'Jelaskan arti layanan RUA di DMARC, lalu prioritaskan pipeline receive -> parse -> analyze yang aman, dengan privasi dan ketahanan terhadap abuse terlebih dahulu.',
    'rua.keypoints.title': 'Poin kunci',
    'rua.keypoints.li1': 'RUA adalah tag DMARC yang menentukan tujuan laporan agregat (URI mailto).',
    'rua.keypoints.li2': 'Layanan RUA menerima email laporan, mem-parse XML, mengagregasi metrik, dan menampilkan dashboard (self-hosted atau pihak ketiga).',
    'rua.keypoints.li3': 'Risiko utama bukan DNS, melainkan penanganan data, isolasi tenant, dan abuse (payload terlalu besar, zip bomb, banjir laporan).',
    'rua.definition.title': 'Kejelasan tingkat definisi',
    'rua.definition.li1.html': 'Di DMARC, <span class="code">rua=</span> adalah "Reporting URI(s) for aggregate data" (daftar URI). Jika tidak ada, laporan agregat tidak dibuat.',
    'rua.definition.li2': 'Laporan agregat berupa XML (RFC 7489 Appendix C) yang merangkum hasil auth/alignment per IP sumber dan jumlah.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> berbeda: laporan forensic/kegagalan yang bisa memuat data per pesan dan lebih sensitif privasi.',
    'rua.priorities.title': 'Checklist prioritas',
    'rua.priorities.note': 'P0 = kredibilitas dasar / P1 = pertahanan operasional / P2 = pemolesan.',
    'rua.p0.title.html': '<strong>P0 - Jangan membuat masalah</strong>',
    'rua.p0.li1.html': 'Verifikasi kepemilikan domain (mis. TXT di <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) sebelum menampilkan data laporan.',
    'rua.p0.li2': 'Perkuat intake: batas ukuran, batas ukuran setelah dekompresi, batas kedalaman arsip berlapis, timeout untuk mencegah zip bomb.',
    'rua.p0.li3': 'Parse XML dengan aman (XXE nonaktif, DTD ditolak) dan validasi schema/semantik; buang laporan bermasalah.',
    'rua.p0.li4': 'Publikasikan aturan privasi + retensi yang jelas (XML mentah singkat atau tidak disimpan; perlakukan IP sebagai sensitif).',
    'rua.p1.title.html': '<strong>P1 - Layak dipertahankan secara operasional</strong>',
    'rua.p1.li1': 'Mode minimisasi data: standar / dianonimkan (IPv4 /24, IPv6 /48) / hanya agregat.',
    'rua.p1.li2': 'Isolasi multi-tenant + kontrol akses (pemisahan level baris, enkripsi, audit log).',
  ...
  add('et', {
    ...
    'rua.strategy.title': 'Strateegia',
    'rua.strategy.body': 'Selgita, mida RUA teenus DMARCis tähendab, ja sea esikohale turvaline receive -> parse -> analyze torustik, kus privaatsus ja väärkasutuse vastupanu on esimesed.',
    'rua.keypoints.title': 'Põhipunktid',
    'rua.keypoints.li1': 'RUA on DMARC silt, mis määrab koondaruannete sihtkohad (mailto URI-d).',
    'rua.keypoints.li2': 'RUA teenus võtab aruandemeilid vastu, parsib XML-i, koondab mõõdikud ja kuvab armatuurlauad (oma süsteemis või kolmanda osapoolena).',
    'rua.keypoints.li3': 'Peamine risk pole DNS, vaid andmekäitlus, tenantide isolatsioon ja väärkasutus (liiga suured payloadid, zip-bombid, aruandeflood).',
    'rua.definition.title': 'Mõisteline selgus',
    'rua.definition.li1.html': 'DMARCis on <span class="code">rua=</span> "Reporting URI(s) for aggregate data" (URI-de loend). Kui puudub, koondaruandeid ei genereerita.',
    'rua.definition.li2': 'Koondaruanded on XML (RFC 7489 Appendix C), mis võtab kokku auth/alignment tulemused allika IP ja loendite kaupa.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> on erinev: forensic/veaaruanded, mis võivad sisaldada sõnumipõhiseid andmeid ja on privaatsuselt tundlikumad.',
    'rua.priorities.title': 'Prioriteetne kontrollnimekiri',
    'rua.priorities.note': 'P0 = miinimumusaldus / P1 = operatiivne kaitse / P2 = viimistlus.',
    'rua.p0.title.html': '<strong>P0 - Ära tee inimestele kahju</strong>',
    'rua.p0.li1.html': 'Kinnita domeeni omandus (nt DNS TXT <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) enne aruandeandmete näitamist.',
    'rua.p0.li2': 'Karasta vastuvõttu: suuruse piirangud, lahtipakkimise piirangud, arhiivi sügavuse piirangud, timeoutid zip-bombide vastu.',
    'rua.p0.li3': 'Parsi XML turvaliselt (XXE välja, DTD tagasi lükatud) ja valideeri skeem/semantika; vigased aruanded kõrvale.',
    'rua.p0.li4': 'Avalda selged privaatsus- ja säilitamisreeglid (toor XML lühiajaliselt või üldse mitte; IP-d käsitleda tundlikuna).',
    'rua.p1.title.html': '<strong>P1 - Operatiivselt kaitstav</strong>',
    'rua.p1.li1': 'Andmete minimeerimise režiimid: standard / anonüümistatud (IPv4 /24, IPv6 /48) / ainult koondid.',
    'rua.p1.li2': 'Mitme tenantiga isolatsioon + ligipääsukontroll (rea-taseme eraldus, krüpteerimine, auditlogid).',
  ...
  add('ko', {
    ...
    'rua.strategy.title': '전략',
    'rua.strategy.body': 'DMARC에서 RUA 서비스의 의미를 명확히 하고, 개인정보와 악용 내성을 우선한 안전한 receive -> parse -> analyze 파이프라인을 먼저 둔다.',
    'rua.keypoints.title': '핵심 포인트',
    'rua.keypoints.li1': 'RUA는 집계 보고서 목적지(mailto URI)를 지정하는 DMARC 태그이다.',
    'rua.keypoints.li2': 'RUA 서비스는 보고서 메일을 수신하고 XML을 파싱해 지표를 집계한 뒤 대시보드를 제공한다(자체 운영 또는 외부).',
    'rua.keypoints.li3': '주요 위험은 DNS가 아니라 데이터 처리, 테넌트 격리, 악용(과대 페이로드, zip 폭탄, 보고서 폭주)이다.',
    'rua.definition.title': '정의 수준의 명확성',
    'rua.definition.li1.html': 'DMARC에서 <span class="code">rua=</span>는 "Reporting URI(s) for aggregate data"(URI 목록)이다. 없으면 집계 보고서가 생성되지 않는다.',
    'rua.definition.li2': '집계 보고서는 XML(RFC 7489 Appendix C)로 소스 IP별 auth/alignment 결과와 카운트를 요약한다.',
    'rua.definition.li3.html': '<span class="code">ruf=</span>는 다르며, 메시지 단위 데이터를 포함할 수 있는 포렌식/실패 보고서로 개인정보 민감도가 높다.',
    'rua.priorities.title': '우선순위 체크리스트',
    'rua.priorities.note': 'P0 = 최소 신뢰성 / P1 = 운영 방어 / P2 = 마무리.',
    'rua.p0.title.html': '<strong>P0 - 문제를 일으키지 않기</strong>',
    'rua.p0.li1.html': '보고서 데이터를 보여주기 전에 도메인 소유를 검증한다(예: <span class="code">_dmarc4all-verify.&lt;domain&gt;</span> TXT).',
    'rua.p0.li2': '수신 하드닝: 크기 제한, 압축 해제 크기 제한, 중첩 깊이 제한, zip 폭탄 차단 타임아웃.',
    'rua.p0.li3': 'XML을 안전하게 파싱(XXE 비활성, DTD 거부)하고 스키마/의미를 검증; 잘못된 보고서는 폐기.',
    'rua.p0.li4': '명확한 개인정보/보관 규칙 공개(원본 XML은 단기 또는 미보관; IP는 민감 정보로 취급).',
    'rua.p1.title.html': '<strong>P1 - 운영적으로 방어 가능하게</strong>',
    'rua.p1.li1': '데이터 최소화 모드: 표준 / 익명화(IPv4 /24, IPv6 /48) / 집계만.',
    'rua.p1.li2': '멀티테넌트 격리 + 접근 제어(행 수준 분리, 암호화, 감사 로그).',
  ...
  add('es', {
    ...
    'rua.strategy.title': 'Estrategia',
    'rua.strategy.body': 'Aclarar que significa un servicio RUA en DMARC y priorizar un flujo seguro receive -> parse -> analyze, con privacidad y resistencia al abuso primero.',
    'rua.keypoints.title': 'Puntos clave',
    'rua.keypoints.li1': 'RUA es la etiqueta DMARC que especifica destinos de reportes agregados (URI mailto).',
    'rua.keypoints.li2': 'Un servicio RUA recibe correos de reporte, analiza XML, agrega metricas y muestra paneles (autoalojado o tercero).',
    'rua.keypoints.li3': 'El riesgo principal no es DNS; es el manejo de datos, el aislamiento multi-tenant y el abuso (cargas enormes, zip bombs, floods de reportes).',
    'rua.definition.title': 'Claridad a nivel de definicion',
    'rua.definition.li1.html': 'En DMARC, <span class="code">rua=</span> es "Reporting URI(s) for aggregate data" (lista de URI). Si falta, no se generan reportes agregados.',
    'rua.definition.li2': 'Los reportes agregados son XML (RFC 7489 Appendix C) que resumen resultados de auth/alignment por IP de origen y conteos.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> es distinto: reportes forenses/de fallos que pueden incluir datos por mensaje y son mas sensibles en privacidad.',
    'rua.priorities.title': 'Lista priorizada',
    'rua.priorities.note': 'P0 = credibilidad minima / P1 = defensa operativa / P2 = pulido.',
    'rua.p0.title.html': '<strong>P0 - Evitar problemas</strong>',
    'rua.p0.li1.html': 'Verificar propiedad del dominio (p. ej., DNS TXT en <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) antes de mostrar datos de reportes.',
    'rua.p0.li2': 'Endurecer la recepcion: limites de tamano, de descompresion, de profundidad de archivos anidados, y timeouts contra zip bombs.',
    'rua.p0.li3': 'Analizar XML de forma segura (XXE deshabilitado, DTD rechazado) y validar esquema/semantica; descartar reportes malformados.',
    'rua.p0.li4': 'Publicar reglas claras de privacidad y retencion (XML bruto corto plazo o ninguno; tratar IPs como sensibles).',
    'rua.p1.title.html': '<strong>P1 - Defendible operativamente</strong>',
    'rua.p1.li1': 'Modos de minimizacion: estandar / anonimizado (IPv4 /24, IPv6 /48) / solo agregados.',
    'rua.p1.li2': 'Aislamiento multi-tenant + control de acceso (separacion por filas, cifrado, logs de auditoria).',
  ...
  add('de', {
    ...
    'rua.strategy.title': 'Strategie',
    'rua.strategy.body': 'Klaren, was ein RUA-Service in DMARC bedeutet, dann eine sichere receive -> parse -> analyze-Pipeline priorisieren, mit Datenschutz und Missbrauchsresistenz zuerst.',
    'rua.keypoints.title': 'Kernpunkte',
    'rua.keypoints.li1': 'RUA ist das DMARC-Tag fur Ziele von Aggregatberichten (mailto-URIs).',
    'rua.keypoints.li2': 'Ein RUA-Service empfangt Report-Mails, parst XML, aggregiert Metriken und zeigt Dashboards (self-hosted oder Drittanbieter).',
    'rua.keypoints.li3': 'Hauptrisiko ist nicht DNS, sondern Datenhandling, Mandantentrennung und Missbrauch (ubergrosse Payloads, Zip-Bombs, Report-Floods).',
    'rua.definition.title': 'Begriffliche Klarheit',
    'rua.definition.li1.html': 'In DMARC ist <span class="code">rua=</span> "Reporting URI(s) for aggregate data" (eine URI-Liste). Fehlt es, werden keine Aggregatberichte erzeugt.',
    'rua.definition.li2': 'Aggregatberichte sind XML (RFC 7489 Appendix C) und fassen Auth/Alignment-Ergebnisse pro Quell-IP und Counts zusammen.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> ist anders: forensische/Fehlerberichte, die nachrichtenspezifische Daten enthalten konnen und datenschutzsensibler sind.',
    'rua.priorities.title': 'Priorisierte Checkliste',
    'rua.priorities.note': 'P0 = Mindestglaubwurdigkeit / P1 = operative Abwehr / P2 = Feinschliff.',
    'rua.p0.title.html': '<strong>P0 - Keine Probleme verursachen</strong>',
    'rua.p0.li1.html': 'Domaininhaberschaft verifizieren (z. B. DNS TXT bei <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>), bevor Reportdaten angezeigt werden.',
    'rua.p0.li2': 'Intake harten: Grossenlimits, Dekompressionslimits, Tiefe verschachtelter Archive, Timeouts gegen Zip-Bombs.',
    'rua.p0.li3': 'XML sicher parsen (XXE aus, DTD abweisen) und Schema/Semantik validieren; fehlerhafte Reports verwerfen.',
    'rua.p0.li4': 'Klare Datenschutz- und Aufbewahrungsregeln veroffentlichen (rohes XML kurzzeitig oder gar nicht; IPs als sensibel behandeln).',
    'rua.p1.title.html': '<strong>P1 - Operativ belastbar machen</strong>',
    'rua.p1.li1': 'Datenminimierung: Standard / anonymisiert (IPv4 /24, IPv6 /48) / nur Aggregate.',
    'rua.p1.li2': 'Mandantentrennung + Zugriffskontrolle (Row-Level-Separation, Verschlusselung, Audit-Logs).',
  ...
  add('ru', {
    ...
    'rua.strategy.title': 'Стратегия',
    'rua.strategy.body': 'Прояснить, что означает сервис RUA в DMARC, затем приоритизировать безопасный конвейер receive -> parse -> analyze, сначала приватность и устойчивость к злоупотреблениям.',
    'rua.keypoints.title': 'Ключевые пункты',
    'rua.keypoints.li1': 'RUA — это тег DMARC, задающий адреса агрегированных отчётов (mailto URI).',
    'rua.keypoints.li2': 'Сервис RUA принимает письма с отчётами, парсит XML, агрегирует метрики и показывает дашборды (самостоятельно или у третьих лиц).',
    'rua.keypoints.li3': 'Главный риск — не DNS, а обработка данных, изоляция арендаторов и злоупотребления (слишком большие payload, zip-бомбы, флуды отчётов).',
    'rua.definition.title': 'Ясность на уровне определения',
    'rua.definition.li1.html': 'В DMARC параметр <span class="code">rua=</span> — это "Reporting URI(s) for aggregate data" (список URI). Если отсутствует, агрегированные отчёты не формируются.',
    'rua.definition.li2': 'Агрегированные отчёты — XML (RFC 7489 Appendix C), суммируют результаты auth/alignment по IP-источникам и количествам.',
    'rua.definition.li3.html': '<span class="code">ruf=</span> отличается: форензик/отчёты о сбоях, могут содержать данные по отдельным сообщениям и более чувствительны к приватности.',
    'rua.priorities.title': 'Приоритетный чек-лист',
    'rua.priorities.note': 'P0 = минимальная достоверность / P1 = операционная защита / P2 = полировка.',
    'rua.p0.title.html': '<strong>P0 - Не создавать проблем</strong>',
    'rua.p0.li1.html': 'Проверять владение доменом (например, TXT в <span class="code">_dmarc4all-verify.&lt;domain&gt;</span>) перед показом данных отчётов.',
    'rua.p0.li2': 'Укрепить приём: лимиты размера, лимиты распаковки, лимиты глубины архивов, тайм-ауты против zip-бомб.',
    'rua.p0.li3': 'Безопасно парсить XML (XXE отключён, DTD запрещён) и валидировать схему/семантику; отбрасывать некорректные отчёты.',
    'rua.p0.li4': 'Публиковать явные правила приватности и хранения (сырые XML краткосрочно или не хранить; IP считать чувствительными).',
    'rua.p1.title.html': '<strong>P1 - Операционно защищаемо</strong>',
    'rua.p1.li1': 'Режимы минимизации данных: стандарт / анонимизация (IPv4 /24, IPv6 /48) / только агрегаты.',
    'rua.p1.li2': 'Изоляция multi-tenant + контроль доступа (разделение по строкам, шифрование, audit-логи).',
  ...
  add('zh', {
    ...
    'rua.strategy.title': '策略',
    'rua.strategy.body': '明确 RUA 服务在 DMARC 中的含义，然后优先构建安全的 receive -> parse -> analyze 流水线，把隐私和滥用抵抗放在第一位。',
    'rua.keypoints.title': '关键要点',
    'rua.keypoints.li1': 'RUA 是 DMARC 用于指定聚合报告目的地的标签（mailto URI）。',
    'rua.keypoints.li2': 'RUA 服务接收报告邮件、解析 XML、聚合指标并展示仪表盘（自建或第三方）。',
    'rua.keypoints.li3': '主要风险不是 DNS，而是数据处理、租户隔离和滥用（超大载荷、zip 炸弹、报告洪泛）。',
    'rua.definition.title': '定义层面澄清',
    'rua.definition.li1.html': '在 DMARC 中，<span class="code">rua=</span> 是 "Reporting URI(s) for aggregate data"（URI 列表）。缺失则不会生成聚合报告。',
    'rua.definition.li2': '聚合报告为 XML（RFC 7489 Appendix C），汇总每个源 IP 的 auth/alignment 结果与数量。',
    'rua.definition.li3.html': '<span class="code">ruf=</span> 不同：取证/失败报告可能包含具体消息数据，隐私敏感度更高。',
    'rua.priorities.title': '优先级清单',
    'rua.priorities.note': 'P0 = 基础可信 / P1 = 运营防御 / P2 = 打磨。',
    'rua.p0.title.html': '<strong>P0 - 不要惹麻烦</strong>',
    'rua.p0.li1.html': '在展示任何报告数据前验证域名所有权（例如在 <span class="code">_dmarc4all-verify.&lt;domain&gt;</span> 设 TXT）。',
    'rua.p0.li2': '加固接收：大小限制、解压后大小限制、嵌套层级限制、超时以阻断 zip 炸弹。',
    'rua.p0.li3': '安全解析 XML（禁用 XXE、拒绝 DTD）并做模式/语义校验；丢弃异常报告。',
    'rua.p0.li4': '明确隐私与保留规则（原始 XML 短期或不保存；IP 视为敏感信息）。',
    'rua.p1.title.html': '<strong>P1 - 具备可运营防御</strong>',
    'rua.p1.li1': '数据最小化模式：标准 / 匿名化（IPv4 /24, IPv6 /48）/ 仅聚合。',
    'rua.p1.li2': '多租户隔离 + 访问控制（行级隔离、加密、审计日志）。',
  ...
  add('km', {
    ...
    'rua.strategy.title': 'យុទ្ធសាស្ត្រ',
    'rua.strategy.body': 'បញ្ជាក់អត្ថន័យនៃសេវា RUA ក្នុង DMARC ហើយផ្តល់អាទិភាពដល់ pipeline receive -> parse -> analyze ដោយដាក់ភាពឯកជន និងការទប់ស្កាត់ការប្រើប្រាស់ខុសជាមុន។',
    'rua.keypoints.title': 'ចំណុចសំខាន់ៗ',
    'rua.keypoints.li1': 'RUA ជា tag DMARC សម្រាប់កំណត់ទីតាំងរបាយការណ៍សរុប (mailto URI)។',
    'rua.keypoints.li2': 'សេវា RUA ទទួលអ៊ីមែលរបាយការណ៍, parse XML, សរុបទិន្នន័យ និងបង្ហាញ dashboard (ដំណើរការដោយខ្លួនឯង ឬភាគីទីបី)។',
    'rua.keypoints.li3': 'ហានិភ័យចម្បងមិនមែន DNS ទេ ប៉ុន្តែជាការគ្រប់គ្រងទិន្នន័យ ការបែងចែក tenant និងការប្រើប្រាស់ខុស (payload ធំ, zip bomb, report flood)។',
    'rua.definition.title': 'ភាពច្បាស់លាស់កម្រិតនិយមន័យ',
    'rua.definition.li1.html': 'ក្នុង DMARC, <span class="code">rua=</span> គឺ "Reporting URI(s) for aggregate data" (បញ្ជី URI)។ ប្រសិនបើគ្មាន នឹងមិនបង្កើតរបាយការណ៍សរុបទេ។',
    'rua.definition.li2': 'របាយការណ៍សរុបជារាង XML (RFC 7489 Appendix C) ដែលសង្ខេបលទ្ធផល auth/alignment តាម IP ប្រភព និងចំនួន។',
    'rua.definition.li3.html': '<span class="code">ruf=</span> ខុសគ្នា៖ របាយការណ៍ forensic/failure អាចមានទិន្នន័យតាមសារមួយៗ និងមានភាពងាយរងគ្រោះផ្នែកឯកជនភាពខ្ពស់។',
    'rua.priorities.title': 'បញ្ជីអាទិភាព',
    'rua.priorities.note': 'P0 = ភាពទុកចិត្តអប្បបរមា / P1 = ការការពារប្រតិបត្តិការ / P2 = ការតុបតែងចុងក្រោយ។',
    'rua.p0.title.html': '<strong>P0 - កុំបង្កបញ្ហា</strong>',
    ...
    'rua.p0.li2': 'ពង្រឹងការទទួល: កំណត់ទំហំ, កំណត់ទំហំបន្ទាប់ពី解압, កំណត់ជម្រៅ archive, និង timeout ទប់ zip bomb។',
    'rua.p0.li3': 'Parse XML ឲ្យមានសុវត្ថិភាព (បិទ XXE, បដិសេធ DTD) និងផ្ទៀងផ្ទាត់ schema/semantics; បោះចោលរបាយការណ៍ខូច។',
    'rua.p0.li4': 'បោះផ្សាយច្បាស់លាស់អំពីឯកជនភាព និងការរក្សាទុក (XML ដើមរក្សាទុករយៈពេលខ្លី ឬមិនរក្សាទុក; ចាត់ទុក IP ជាទិន្នន័យងាយរងគ្រោះ)។',
    'rua.p1.title.html': '<strong>P1 - ការពារបានក្នុងប្រតិបត្តិការ</strong>',
    'rua.p1.li1': 'ម៉ូដកាត់បន្ថយទិន្នន័យ៖ ស្តង់ដារ / អនាមិក (IPv4 /24, IPv6 /48) / សរុបតែប៉ុណ្ណោះ។',
    'rua.p1.li2': 'ការបែងចែក multi-tenant + ការគ្រប់គ្រងការចូល (បែងចែកជួរ, អ៊ិនគ្រីប, audit logs)។',
  ...
  add('my', {
    ...
    'rua.links.spec': 'Service spec (docs)',
    'rua.strategy.title': 'မဟာဗျူဟာ',
    'rua.strategy.body': 'DMARC အတွင်း RUA ဝန်ဆောင်မှု၏အဓိပ္ပါယ်ကိုရှင်းလင်းပြီး receive -> parse -> analyze ပိုင်းလိုင်းကို လုံခြုံရေး၊ ပရိုက်ဗစီနှင့် အလွဲသုံးစားမှုတားဆီးရေးကို ဦးစားပေးရန်။',
    'rua.keypoints.title': 'အချက်အလက်အဓိက',
    'rua.keypoints.li1': 'RUA သည် စုပေါင်းအစီရင်ခံချက်လက်ခံရာနေရာကို သတ်မှတ်하는 DMARC tag (mailto URI) ဖြစ်သည်။',
    'rua.keypoints.li2': 'RUA ဝန်ဆောင်မှုသည် အစီရင်ခံအီးမေးလ်များကို လက်ခံ၍ XML ကို parse လုပ်ပြီး မက်ထရစ်များကို စုပေါင်းကာ ဒက်ရှ်ဘုတ်တွင်ပြသည် (ကိုယ်ပိုင် သို့မဟုတ် တတိယပါတီ)။',
    'rua.keypoints.li3': 'အဓိကအန္တရာယ်မှာ DNS မဟုတ်ဘဲ ဒေတာကိုင်တွယ်မှု၊ tenant ခွဲခြားမှုနှင့် အလွဲသုံးစားမှု (payload ကြီးခြင်း, zip bomb, report flood) ဖြစ်သည်။',
    'rua.definition.title': 'အဓိပ္ပါယ်အဆင့် ရှင်းလင်းမှု',
    'rua.definition.li1.html': 'DMARC တွင် <span class="code">rua=</span> သည် "Reporting URI(s) for aggregate data" (URI စာရင်း) ဖြစ်သည်။ မရှိပါက စုပေါင်းအစီရင်ခံချက်မထုတ်ပါ။',
    'rua.definition.li2': 'စုပေါင်းအစီရင်ခံချက်များမှာ XML (RFC 7489 Appendix C) ဖြစ်ပြီး source IP အလိုက် auth/alignment ရလဒ်နှင့် အရေအတွက်ကို သတ်မှတ်သေးသည်။',
    'rua.definition.li3.html': '<span class="code">ruf=</span> သည်ကွာခြားပြီး forensic/failed အစီရင်ခံချက်ဖြစ်နိုင်ပြီး စာတစ်စောင်ချင်းဒေတာ ပါဝင်နိုင်သဖြင့် ပိုမိုပရိုက်ဗစီနဲ့敏感 ဖြစ်သည်။',
    'rua.priorities.title': 'ဦးစားပေးစာရင်း',
    'rua.priorities.note': 'P0 = အခြေခံယုံကြည်ရမှု / P1 = လက်တွေ့ကာကွယ်မှု / P2 = အလှဆင်.',
    'rua.p0.title.html': '<strong>P0 - ပြဿနာမဖြစ်စေရန်</strong>',
    ...
    'rua.p0.li2': 'လက်ခံခြင်းကို ခိုင်မာစေ: အရွယ်အစားကန့်သတ်ချက်,解压 အရွယ်အစားကန့်သတ်ချက်, archive အနက်ကန့်သတ်ချက်, zip bomb တားဆီး timeout။',
    'rua.p0.li3': 'XML ကို လုံခြုံစွာ parse (XXE ပိတ်, DTD ငြင်းပယ်) နှင့် schema/semantics စစ်ဆေး; မှားယွင်းသောအစီရင်ခံချက်များကို ဖျက်ပစ်။',
    'rua.p0.li4': 'ပရိုက်ဗစီနှင့် ထိန်းသိမ်းမှုစည်းမျဉ်းများကို ပြတ်သားစွာ ထုတ်ပြန် (XML ဒေတာကို အချိန်တို/မသိမ်း; IP ကို အရေးကြီးဒေတာအဖြစ်ယူဆ)။',
    'rua.p1.title.html': '<strong>P1 - လက်တွေ့ကာကွယ်နိုင်မှု</strong>',
    'rua.p1.li1': 'ဒေတာလျှော့ချပေးရေး မုဒ်များ: စံ / အမည်မဖော် (IPv4 /24, IPv6 /48) / စုပေါင်းသာ။',
    'rua.p1.li2': 'Multi-tenant ခွဲခြားမှု + အာမခံဝင်ရောက်ခွင့် (row-level ခွဲခြား, စာဝှက်, audit logs)။',
